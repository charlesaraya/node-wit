...
function Wit (opts) {
  ...
  const { accessToken, apiVersion, actions, headers, logger, witURL } = this.config = Object.freeze(validate(opts))
  ...
  this.converse = (sessionId, message, context, reset) => {
    ...
    return fetch(fullURL, { method, headers, body: JSON.stringify(context) })
      .then(response => Promise.all([response.json(), response.status]))
      .then(handler)
  }
    ...
    return (json) => {
      ...
      if (json.type === 'msg') {
        ...
        return actions[json.action](request).then(ctx => {
          ...
          return this.converse(sessionId, null, nextContext).then(continueRunActions(sessionId, currentRequest, message, nextContext, i - 1)
          )
        })
      } else {
...
const validate = (opts) => {
  if (!opts.accessToken) {
    throw new Error("Couldn't find access token, learn more at https://wit.ai/docs")
  }
...
const validateActions = (logger, actions) => {
  ...
  Object.keys(actions).forEach(key => {
    ...
    if ((key === 'say' || key === 'merge' || key === 'error') && actions[key].length > 2) {
